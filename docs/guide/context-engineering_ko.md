...
OmA에서 이 개념은 단순한 "좋은 프롬프트 작성"이 아니라, 다음을 포함한 운영 모델입니다.
...
- `/oma:status`: 스테이지/모드/진행 상태 확인
- `/oma:cache`: 캐시 안정성 리스크 점검
- `/oma:optimize`: 컨텍스트 비대화/변동성 최적화
- `/oma:checkpoint`: 재개용 요약 상태 저장
