...
- OmA 권장 런타임 baseline: Gemini CLI `v0.42.0+`.
  - 현재 stable의 extension, subagent, policy, session, sandbox, model-selection 동작을 기준으로 합니다.
  - preview/nightly 빌드는 선택 사항이며, OmA의 일반 동작에는 preview/nightly 전용 기능이 필요하지 않습니다.
  - 2026년 5월 upstream changelog 기준으로 OmA command, hook, agent, manifest 코드 변경은 필요하지 않습니다.
...
- Subagent 호환성:
  ...
  - OmA는 delegated/worker/subagent turn을 shared workflow state에 대해 read-mostly로 유지합니다.
  - shared OmA state는 계속 `.omg/state/session-lock.json` 기반 단일 orchestration writer를 전제로 합니다.
...
- Model routing 호환성:
  ...
  - OmA `balanced`는 deterministic routing을 위해 명시적인 lane model ID를 계속 씁니다.
  - `/oma:model auto`는 lane model 선택을 Gemini CLI runtime auto-model policy에 위임합니다.
- Policy와 approval 호환성:
  ...
  - OmA goal/autopilot 흐름은 Gemini CLI approval, sandbox, trusted-folder, shell, network, policy 경계를 우회하지 않습니다.
...
- Hook 호환성:
  ...
  - OmA는 quiet `BeforeModel` router와 `AfterAgent` learn-signal safety filter만 유지합니다.
...
- Memory 호환성:
  ...
  - OmA `/oma:memory`는 `MEMORY.md`, `.omg/memory/*`, path-aware rule pack을 다루는 project workflow 명령으로 유지하며, Gemini CLI native memory review의 대체물로 취급하지 않습니다.
- Browser-agent 참고:
  - Gemini CLI는 `browser_agent`를 experimental로 문서화하고 있습니다.
  - OmA는 기본적으로 `browser_agent`를 활성화하거나 의존하지 않습니다.
- Slash planning 호환성:
  - Gemini CLI native planning은 `/plan`입니다.
  - OmA planning skill은 `/oma-plan` 또는 `$oma-plan`을 사용합니다.
  - staged OmA workflow는 `/oma:team-plan`, `/oma:team-assemble`, `/oma:team`을 사용합니다.
## 인터페이스 맵
...
ì¤í¸ ìì§ëì´ë§ ìë¦¬
- [everything-claude-code](https://github.com/affaan-m/everything-claude-code) - Claude Code CLI íë¤ì¤
